async function bubblesort() {
    ...
    for (let i = 0; i < array.length - 1; i++) {
        for (let j = 0; j < array.length - i - 1; j++) {
            ...
            await new Promise((resolve) => setTimeout(resolve, arraySpeed)); // Wait for visualization
        ...
        // at the end element sorted will be orange
        bars[array.length - i - 1].style.backgroundColor = "orange";
    }
    // first element remaining so make orange finally
...
async function swapBarsforbubble(index1, index2) {
    ...
    // Reseting transforms to avoid visual glitches
